<?php
	class Menu {
		...
		public static function allWS($group, $sort) {
			$result = DB::query("SELECT DISTINCT MH.namamenu AS nama, M.jumlahtersedia, M.deskripsi, M.harga, K.nama AS kategori FROM MENU_HARIAN MH, MENU M, KATEGORI K WHERE MH.namamenu = M.nama AND M.kategori = K.kode ORDER BY $group $sort");
			if($result == false || $result->rowCount() <= 0) {
				return null;
			} else {
				$menus = array();
